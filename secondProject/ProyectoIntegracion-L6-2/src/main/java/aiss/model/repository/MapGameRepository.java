...
public class MapGameRepository implements GameRepository{
	...
	public void iniData() {
		Stream<Genre> genres = MapGenreRepository.getInstance().getAllGenre().stream();
		Stream<Developer> devs = MapDeveloperRepository.getInstance().getAllDevelopers().stream();
		Stream<Platform> p = MapPlatformRepository.getInstance().getAllPlatforms().stream();
		List<Platform> lP;
		List<Mode> lM;
		List<Genre> lG;
		...
		lP.add(p.filter(x->x.getName().toLowerCase().contains("switch")).findFirst().get());
		lM.add(Mode.Individual);
		lM.add(Mode.Cooperativo_local);
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("platform")).findFirst().get());
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("adventure")).findFirst().get());
		...
		g1.setYear(2017);
		g1.setDeveloper(devs.filter(x->x.getName().toLowerCase().contains("nintendo")).findFirst().get());
		g1.setScore(9.5);
		g1.setPlatforms(lP);
		g1.setModes(lM);
		g1.setGenres(lG);
		...
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("simulation")).findFirst().get());
		...
		g2.setYear(2020);
		g2.setDeveloper(devs.filter(x->x.getName().toLowerCase().contains("nintendo")).findFirst().get());
		g2.setScore(9.5);
		g2.setPlatforms(lP);
		g2.setModes(lM);
		g2.setGenres(lG);
		...
		lP.addAll(p.filter(x->x.getName().toLowerCase().contains("xbox")).collect(Collectors.toList()));
		lP.addAll(p.filter(x->x.getName().toLowerCase().contains("playstation")).collect(Collectors.toList()));
		lM.add(Mode.Individual);
		lM.add(Mode.Multijugador);
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("action")).findFirst().get());
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("adventure")).findFirst().get());
		...
		g3.setYear(2013);
		g3.setDeveloper(devs.filter(x->x.getName().toLowerCase().contains("rockstar")).findFirst().get());
		g3.setScore(9.5);
		g3.setPlatforms(lP);
		g3.setModes(lM);
		g3.setGenres(lG);
		...
		lP.addAll(p.filter(x->x.getName().toLowerCase().contains("xbox")).collect(Collectors.toList()));
		lP.add(p.filter(x->x.getName().toLowerCase().contains("switch")).findFirst().get());
		lP.add(p.filter(x->x.getName().toLowerCase().contains("personal")).findFirst().get());
		
		lM.add(Mode.Individual);
		lM.add(Mode.Multijugador);
		
		lG.add(genres.filter(x->x.getName().toLowerCase().contains("sports")).findFirst().get());
		...
		g4.setYear(2019);
		g4.setDeveloper(devs.filter(x->x.getName().toLowerCase().contains("electronic arts")).findFirst().get());
		g4.setScore(8.1);
		g4.setPlatforms(lP);
		g4.setModes(lM);
		g4.setGenres(lG);
