...
/**
 *
 * This class loads the data from the OAuth providers and creates authorization
 * flows for our servlets.
 *
 * @author japarejo
 *
 */
public class OAuthRegistry {
    ...
    /**
     * Creates an OAuth2 authorization flow for using the data provided as
     * parameter
     *
     * @param tokenURL
     * @param clientID
     ...
     * @return an authorization flow configured for the service provided as
     * parameter
     * @throws IOException when there is no configuration for the service in the
     * registry
     */
    ...
    /**
     * Load the configuration of the different OAuth providers from the Json
     * file.
     */
    private static void loadConfig() {
        ...
        try {
            ...
            InputStream is = Thread.currentThread().getContextClassLoader().getResourceAsStream(OAUTH_RESOURCE);
            ...
            for (Entry<String, OAuthServiceConfiguration> sc : serviceConfigurations.entrySet()) {
                log.info("Loaded config for " + sc.getKey());
            }
            log.info(serviceConfigurations.size() + " OAuth configurations have been loaded!");
